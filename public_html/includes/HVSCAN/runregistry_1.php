...
if($_GET['type'] == 'current') {
    $TYPESCAN = 'current';
    $TITLE = 'CURRENT';
    $sth1 = $dbh->prepare("SELECT h.*, d.type AS scantype FROM hvscan h, hvscan_CURRENT d WHERE h.type = 'current' AND h.id = d.id  ORDER BY id DESC");
}
    ...
    if(isset($_GET['label'])) {
        ...
        $sth1 = $dbh->prepare("SELECT h.*, d.type AS scantype FROM hvscan h, hvscan_DAQ d WHERE h.label = '".$_GET['label']."' AND h.id = d.id ORDER BY id DESC");
    }
    else die();
...
$(function() {
  ...
  $(".RunRegistrySorter").tablesorter({
	...
	headers: { 0: { sorter: false, filter: false }, 2: { sorter: false, filter: false }, 3: { sorter: false, filter: false } },
...
<table class="RunRegistrySorter" cellpadding="5px" cellspacing="0">
    ...
    <thead><tr>
        ...
        <th class="oddrow" width="105px">Trolley 1 pos.</th>
        <th class="oddrow filter-select" data-placeholder="Source" width="50px">Source</th>
        <th class="oddrow filter-select" data-placeholder="Att. U" width="60px">Att. U</th>
        <th class="oddrow filter-select" data-placeholder="Att. D" width="60px">Att. D</th>
        <th class="oddrow filter-select" data-placeholder="Beam" width="60px">Beam</th>
