...
if(filter_input(INPUT_POST, 'startscan') != NULL) {	
    ...
    if($error == "") {
        ...
        $sth1 = $DB['MAIN']->prepare("INSERT INTO hvscan (id, time_start,  type, waiting_time,  comments,  maxHVPoints, status, RPC_mode, measure_intval,  measure_time,  lastHV)
                              VALUES (               :id, :time_start, :type, :waiting_time, :comments, :maxHVPoints, 1, :RPC_mode, :measure_intval, :measure_time, :lastHV) "); 
        ...
        if($TYPESCAN == 'current') {
            ...
            $sth2 = $DB['MAIN']->prepare("INSERT INTO hvscan_DAQ (id, type, trigger_mode) VALUES (:id, :type, :trigger_mode) ");
            $sth2->bindParam(':id', $id);
            $sth2->bindParam(':type', $scantype);
