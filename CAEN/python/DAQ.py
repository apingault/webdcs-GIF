...
if opts.id is None:
    parser.error('Please provide input ID')
scanid = opts.id
...
class LYONDAC():
    ...
    def status(self):
        return 0  # always OK
        try:
            urllib2.urlopen(self.CMD_status).read()
        except urllib2.HTTPError as e:
            return 2  # SECOND CHECK: 404
        except:
            return 1  # CHECK IF SERVER EXIST
        return 0  # ok
...
if opts.init:
    ...
    if DAQTYPE == "digitizer":
        log("Generate first digi_daq.ini file")
        call("python /home/webdcs/software/webdcs/CAEN/python/generateDAQFiles.py --id " + str(opts.id) + " --daqini --HV 1 --maxtriggers 5000", shell=True)
...
if opts.start:
    if DAQTYPE == "default":
        call(
            "/home/webdcs/software/GIF_DAQ/bin/daq " + LOG + " > logdaq_" + str(opts.id) + ".txt 2>&1 &",
            shell=True)
    ...
    if DAQTYPE == "digitizer":
        ...
        log("Starting digitizerdaq...")
        call("cd /home/webdcs/software/DigitizerDaq/; python digidaq.py config/config --useDcs  > logDigidaq_" + str(opts.id) + ".txt 2>&1 &", shell=True)
